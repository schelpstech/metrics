...
if (!empty($transaction_details)) {
  ...
  echo 'Transaction with reference number : ' . $reference . ' not found.';
}
...
curl_close($curl);
if ($err) {
  echo 'Error verifying this transaction, kindly send us an email to resolve this issue.';
} else {
...
if ($verify_response['status'] == true) {
  ...
  if (($paymentstatus ==  $status)
    ...
    if ($update) {
      ...
      echo 'Unable to update transaction records, kindly contact us via email.';
    }
  } else {
    echo 'Payment verification failed. Kindly contact us via email.';
  }
} else {
  echo 'Payment verification failed. Kindly contact us via email.';
}        
